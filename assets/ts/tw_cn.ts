...
const initializeTranslation = () => {
    ...
    function translateBody(fobj?: HTMLElement) {
        const objs: NodeListOf<ChildNode> = fobj?.childNodes || document.body.childNodes;

        objs.forEach((raw) => {
            const obj = raw as HTMLElement & { alt?: string; placeholder?: string; data?: string };
            ...
            if (obj.tagName === 'INPUT' && (obj as HTMLInputElement).value && !['text', 'hidden'].includes((obj as HTMLInputElement).type || '')) {
                (obj as HTMLInputElement).value = translateText((obj as HTMLInputElement).value || '');
            }
            if (obj.nodeType === 3) {
